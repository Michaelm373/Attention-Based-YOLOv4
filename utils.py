...
"""
many utils are taken and modified from Aladin Persson's implementation of YOLOv3 linked below
...
def check_class_accuracy(model, data, threshold, device):
...
def plot_examples(model, data, thresh, iou_thresh, anchors, device, labels):
    model.eval()
    x, y = data[0], data[1]
    if device == "cuda":
        x = x.to("cuda")
    ...
    for i in range(batch_size):
        ...
        plot_image(x[i].permute(1, 2, 0).detach().cpu(), nms_boxes, labels)
